# HerNext Network, copy deck

Every line of text on the site, organised by page and section, so wording can be
reviewed and revised here before it is applied to the HTML.

**Source.** Substance is drawn from the four HerNext Network correction
documents of 2026: the About page revision, and the full website corrections for
Our Work, Our Impact, and Partners and Collaboration. Those documents supply the
institutional language but no taglines, calls to action, form labels, error
messages or microcopy. Those were written for this site and are marked **[new]**
below.

Copy that predates the corrections and came from the Institutional Profile,
First Edition 2026, is marked **[profile]** where it survives unchanged.

**House style.**

- No em dashes. Use a comma, a colon, or the word *and*. The correction
  documents use em dashes and ampersands freely; both are re-typeset here.
- Ampersands are spelled out. *Enterprise and Livelihood Development*, not
  *Enterprise & Livelihood Development*.
- British spelling: organisation, recognise, programme, centred.
- One italic emphasis word per heading, no more.
- Sentence case for headings. Title case only for proper names of initiatives.
- Numerals `01` to `09` for sequences, spelled-out numbers in prose.

---

## Global

### Brand
- **Name:** HerNext Network
- **Motto:** Creating Opportunity. Building Legacy.
- **Descriptor:** A women-centred Pan-African institution

### Navigation
About · Our Work · Impact · Partners · Contact

Five links, no call to action button. Registration goes through the contact
form, so a sixth item would only duplicate Contact.

### Footer
- **Motto line:** Creating Opportunity. Building Legacy.
- **Explore:** About HerNext · Our Work · Impact · Partners
- **Engage:** Current opportunities · Become a partner · Contact us
- **Contact:** info@hernextnetwork.com · hernextnetwork@gmail.com ·
  0780 528 551 · 0734 806 637 · Instagram @hernextnetworkltd ·
  LinkedIn *(still a placeholder)*
- **Legal line:** Privacy · Terms · Accessibility
- **Legal:** Copyright HerNext Network. All rights reserved.

Telephone numbers display in Kenyan local format and dial in international
format, `tel:+254780528551` and `tel:+254734806637`.

### Closing call to action *(repeated on Home, About, Our Work, Impact)* **[new]**
- **Label:** Take the next step
- **Heading:** Tell us what you are *building*
- **Body:** Whether you are a woman building an enterprise, an organisation
  seeking meaningful collaboration, or an investor, buyer, technical expert or
  development partner, we welcome the conversation. Registering your interest
  tells us what to build, and means you hear from us as initiatives open.
- **Buttons:** Register your interest · Partner with us

### The HerNext pathway
The eight-step chain appears on Home, Our Work and, in its longer form, on
Impact. It is the spine of the whole site and its wording should not drift.

> Listen → Identify → Design → Partner → Implement → Measure → Learn → Scale

---

## Home

### Hero
- **Label:** Pan-African institution
- **Heading:** Creating opportunity. Building *legacy*.
- **Lead:** HerNext Network is a women-centred Pan-African institution advancing
  women's economic transformation through practical, evidence-driven
  interventions that expand access to markets, finance, skills, strategic
  partnerships and sustainable livelihood opportunities.
- **Buttons:** Register your interest · Explore our work
- **Meta strip:** Six areas of work · Six signature initiatives ·
  Nine priority sectors

### Why we exist
- **Label:** Why we exist
- **Statement:** Our ambition is not simply to reach more women. It is to
  improve economic *outcomes*.
- **Body:** Across the continent, women build businesses, strengthen
  agricultural value chains, lead organisations, create employment and drive
  innovation. Yet many remain constrained by barriers to markets, finance,
  technology, productive resources, skills and strategic networks. HerNext
  Network was established to help bridge these gaps.
- **Link:** Read our story

### Our identity **[new]**
- **Heading:** Six commitments that shape *everything* we do
- **Lead:** HerNext serves as a platform for collaboration, where evidence
  becomes partnerships, partnerships become interventions, and interventions
  produce measurable economic change.

| # | Card | Text |
|---|---|---|
| 01 | Women-centred | Women stay at the centre of our positioning, our design and our measurement, not at the edge of it. |
| 02 | Evidence-driven | We listen before prescribing, and build interventions around demonstrated needs rather than assumptions. |
| 03 | Impact-focused | We measure success by what changed in enterprises, incomes and livelihoods, not by activities completed. |
| 04 | Partnership-driven | Complex economic barriers cannot be solved alone. We organise, connect, coordinate and measure. |
| 05 | Africa-centred | Our work reflects Africa's own realities and priorities while embracing global partnerships. |
| + | Built for the long term | We prioritise solutions capable of creating value beyond a single activity or funding cycle. |

The first three are the client's own strapline: women-centred, evidence-driven,
impact-focused.

### How we work **[new]**
- **Heading:** We listen before we *intervene*
- **Body:** Every HerNext intervention follows the same path, from the first
  conversation with a woman about her enterprise through to the measurement that
  shows whether anything actually changed.
- **Pathway:** Listen → Identify → Design → Partner → Implement → Measure →
  Learn → Scale
- **Link:** See how we work

### Signature initiatives
- **Heading:** Turning evidence into *action*
- **Lead:** Six initiatives translate our mission into practical action. Each is
  developed and launched on the basis of demonstrated need, strategic
  partnerships and the resources required to deliver meaningful outcomes.
- Six entries, wording as on Our Work below.
- **Link:** Explore our work

### Our operating philosophy
- **Statement:** Impact before visibility. Evidence before intervention.
  Sustainability before *scale*.
- **Body:** Creating opportunity is not simply a social responsibility. It is an
  economic imperative. We do not create programmes simply to create activity: we
  build interventions around demonstrated needs and measure whether they create
  meaningful change.

### Figures
| Value | Label |
|---|---|
| 06 | Interconnected areas of work |
| 06 | Signature initiatives in development |
| 09 | Priority sectors across Africa's growth economy |
| 05 | Sustainable Development Goals supported |

These count structure, not results. No participant number, funding figure or
impact statistic belongs in this band until follow-up measurement supports it.

### Where we work
- **Heading:** Where opportunity *compounds*
- **Body:** We work across economic sectors where women are building
  enterprises, creating employment and contributing to Africa's economic
  transformation, guided by demonstrated need, partnership opportunities and the
  potential to create measurable economic impact.
- Nine sector tags, wording as on Our Work below.

---

## About

### Hero
- **Label:** About HerNext
- **Heading:** A platform for *collaboration*
- **Lead:** HerNext Network is a women-centred Pan-African institution advancing
  women's economic transformation through practical, evidence-driven
  interventions.

### Our story
- **Heading:** Every institution begins with a *purpose*
- We work with women, communities and strategic partners to identify barriers to
  economic progress and build pathways to markets, finance, skills, technology,
  productive resources and sustainable livelihood opportunities. Our work is
  driven by a simple commitment: to turn opportunity into measurable and lasting
  economic impact.
- HerNext began with a simple but important belief: Africa's future will be
  stronger when more women have access to the opportunities, resources and
  systems that enable them to thrive.
- Across the continent, women build businesses, strengthen agricultural value
  chains, lead organisations, create employment and drive innovation. Yet many
  remain constrained by barriers to markets, finance, technology, productive
  resources, skills and strategic networks. HerNext Network was established to
  help bridge these gaps.
- **Rather than assuming what women need, we listen first.** We use evidence to
  understand the barriers affecting their economic progress, build partnerships
  around those needs and develop practical interventions whose impact can be
  measured.
- Our ambition is not simply to reach more women. It is to help create
  measurable improvements in enterprises, livelihoods, incomes and economic
  opportunity.

### Our foundation
- **Heading:** The direction that shapes every *decision*
- **Lead:** Our vision, mission, purpose and philosophy guide how we identify
  needs, design interventions, build partnerships and measure impact.

| Term | Text |
|---|---|
| Vision | An Africa where women have equitable access to the opportunities, resources and markets they need to build prosperous enterprises, sustainable livelihoods and lasting legacies. |
| Mission | To advance women's economic transformation across Africa through practical, evidence-driven interventions that expand access to markets, finance, skills, strategic partnerships and sustainable livelihood opportunities. |
| Purpose | To remove barriers that limit women's economic progress and strengthen the systems that enable enterprises, livelihoods and communities to thrive. |
| Our belief | Opportunity has the power to transform lives, strengthen enterprises, build resilient communities and accelerate economic development. When women have equitable access to markets, finance, knowledge, technology and productive resources, they become powerful drivers of enterprise, innovation, employment and sustainable growth. |
| Our philosophy | Impact before visibility. Evidence before intervention. Sustainability before scale. We listen before prescribing solutions. We use evidence to understand the barriers women face, develop interventions around real needs, measure what changes and scale approaches that demonstrate meaningful and sustainable impact. |

*"Our promise" was dropped by the 2026 correction and is not to be restored.*

### Our core values
- **Heading:** Values define who we are and guide how we *work*
- **Lead:** They influence every partnership we build, every intervention we
  design and every decision we make.

| # | Value | Text |
|---|---|---|
| 01 | Integrity | We uphold the highest standards of honesty, transparency and accountability in everything we do. |
| 02 | Excellence | We pursue quality, professionalism and continuous improvement in our work. |
| 03 | Collaboration | We build partnerships around shared challenges, complementary strengths and measurable outcomes. |
| 04 | Innovation | We embrace new ideas, technologies and approaches that can unlock economic opportunity. |
| 05 | Inclusion | We work to ensure opportunity is accessible and equitable, particularly for women who remain underserved by existing systems. |
| 06 | Sustainability | We develop approaches designed to create lasting economic, social and environmental value. |
| 07 | Accountability | We take responsibility for our commitments, resources and results. |
| 08 | Impact | We measure success not simply by activities completed or people reached, but by the lasting economic difference created through our work. |

*Value 05 was "Inclusivity" before the correction and is now "Inclusion".*

### The Africa we envision
- **Heading:** A future built on *opportunity*
- Africa is a continent of extraordinary potential, home to dynamic economies,
  expanding markets, abundant resources and a new generation of entrepreneurs,
  innovators and leaders.
- Yet Africa's future will depend not simply on the potential of its people, but
  on whether that potential can access the systems and opportunities required to
  flourish. We envision an Africa where women are able to participate fully in
  economic life, as producers, entrepreneurs, innovators, investors, employers
  and leaders.

| Column | Text |
|---|---|
| Women leading | Women building thriving enterprises, strengthening agricultural and industrial value chains, creating employment, advancing innovation and participating in economic decision-making across every sector. |
| Institutions collaborating | Governments, businesses, financial institutions, academia, development partners and communities working together to remove barriers and create functioning pathways to economic opportunity. |
| Prosperity that endures | Economic transformation that increases incomes, strengthens enterprises, creates employment and builds resilient communities for present and future generations. |

### HerNext perspective
> Opportunity creates possibility. Evidence shows us where to act. Impact tells
> us whether we *succeeded*.

*The same quotation closes the Impact page. Both correction documents specify
it, and it is the site's single most repeated line. Keep the two in sync.*

---

## Our Work *(our-work.html)*

Renamed from Opportunities. Current opportunities is a subsection here, not the
identity of the page.

### Hero
- **Label:** Our work
- **Heading:** Turning evidence into *action*
- **Lead:** HerNext develops practical, evidence-driven interventions that
  address barriers affecting women's economic progress.

### What we do
- **Heading:** From barriers to *opportunity*
- **Lead:** Our work begins by listening to women, enterprises and communities,
  understanding the barriers limiting their progress and working with strategic
  partners to develop solutions that can produce measurable and sustainable
  economic outcomes.
- **Callout:** We do not create programmes simply to create activity. We build
  interventions around demonstrated needs and measure whether they create
  meaningful change. Our work is organised around six interconnected areas.

| # | Area | Text |
|---|---|---|
| 01 | Enterprise and Livelihood Development | We support women to build stronger enterprises and sustainable livelihoods by addressing practical barriers to productivity, profitability, resilience and growth. Interventions may include enterprise development, technical support, business skills, productivity improvement and access to productive resources. |
| 02 | Markets and Trade | A viable enterprise needs customers. We connect women producers and entrepreneurs to domestic, regional and international markets through buyer linkages, value chain partnerships, trade opportunities, exhibitions and business to business connections. Our focus is not simply market exposure, but market access that can translate into sustainable commercial opportunity. |
| 03 | Finance and Investment | Access to appropriate capital can enable businesses to invest, expand and compete. We work with financial institutions, investors and development partners to strengthen women's access to suitable financing, investment readiness and financial capability. |
| 04 | Skills, Leadership and Enterprise Capability | Knowledge and leadership strengthen women's ability to build enterprises, manage organisations and participate effectively in economic decision-making. HerNext facilitates practical learning, mentorship, technical capacity development, business capability and leadership development aligned to identified needs. |
| 05 | Innovation, Technology and Sustainability | We promote the adoption of technology, innovation and sustainable practices that can improve productivity, competitiveness, market access and enterprise resilience. |
| 06 | Partnerships and Economic Ecosystems | Complex economic barriers cannot be solved by one institution. HerNext brings together businesses, governments, financial institutions, development partners, academia, technical experts and communities to develop collaborative solutions around clearly identified economic challenges. |

### How we work
- **Heading:** We listen before we *intervene*
- **Lead:** Our approach is designed to ensure that programmes respond to real
  needs rather than assumptions.
- **Pathway:** Listen → Identify → Design → Partner → Implement → Measure →
  Learn → Scale

| # | Step | Text |
|---|---|---|
| 01 | Listen | We engage women, enterprises and communities to understand their realities, ambitions and challenges. |
| 02 | Identify | We collect and analyse evidence to determine the barriers most significantly affecting economic progress. |
| 03 | Design | We develop targeted interventions around the needs identified. |
| 04 | Partner | We mobilise organisations with the expertise, resources, markets, technology or financing required to address those barriers. |
| 05 | Implement | We deliver practical interventions with clearly defined objectives and intended outcomes. |
| 06 | Measure | We track whether the intervention creates meaningful changes in productivity, income, profitability, market access, business performance or other relevant outcomes. |
| 07 | Learn and scale | We use evidence and participant feedback to improve our approach and scale models that demonstrate meaningful and sustainable impact. |

### Signature initiatives
- **Heading:** Building programmes for measurable *impact*
- **Lead:** HerNext is developing a portfolio of initiatives that translate our
  institutional mission into practical action. Initiatives are developed and
  launched based on demonstrated needs, strategic partnerships and the resources
  required to deliver meaningful outcomes.

| Initiative | Text |
|---|---|
| HerNext Leadership Academy | A leadership and enterprise development initiative designed to strengthen women's capability to lead organisations, businesses and communities. |
| HerNext Opportunity Hub | A platform connecting women to relevant business opportunities, mentorship, markets, finance, technical expertise and strategic networks. |
| HerNext Global Trade and Market Access Platform | An initiative connecting women-owned enterprises and producers with buyers, distributors, trade missions, exhibitions and regional and international market opportunities. |
| HerNext Enterprise and Livelihoods Initiative | A practical economic development initiative supporting women entrepreneurs, producers and farmers to overcome barriers affecting productivity, profitability, market access and sustainable livelihoods. |
| HerNext Innovation and Sustainability Forum | A platform bringing together innovators, enterprises and institutions to explore practical applications of technology, innovation and sustainability in Africa's economic development. |
| Research, Evidence and Policy Dialogue | HerNext uses evidence generated through its programmes and partnerships to contribute to knowledge, dialogue and policy conversations concerning women's economic participation, enterprise development and inclusive economic transformation. |

- **Callout:** **These initiatives are in development.** None is currently
  accepting applications. Where an initiative opens, it will appear under
  current opportunities below.

*"HerNext Partnership Forum" was removed by the 2026 correction. Do not
reintroduce it, and do not describe any initiative as open unless it is.*

### Where we work
- **Heading:** Engaging Africa's growth *sectors*
- **Lead:** HerNext works across economic sectors where women are building
  enterprises, creating employment and contributing to Africa's economic
  transformation. Our sector engagement is guided by demonstrated need,
  partnership opportunities and the potential to create measurable economic
  impact.

| Sector | Text |
|---|---|
| Agriculture and Agribusiness | Livestock, food systems, value addition, coffee, flowers, agro-processing and agricultural market linkages. |
| Manufacturing and Industrial Development | Value addition, manufacturing, production and women's participation in industrial value chains. |
| Trade and Export Development | Domestic, regional and international market access, buyer linkages and trade readiness. |
| Technology and Innovation | Digital transformation, technology-enabled enterprise and innovation. |
| Finance and Investment | Financial inclusion, investment readiness and enterprise finance. |
| Energy, Climate and Sustainability | Renewable energy, green enterprise, climate resilience and sustainable production. |
| Healthcare and Life Sciences | Enterprise, innovation and partnerships that contribute to stronger health systems and economic opportunity. |
| Fashion, Textiles and Creative Economy | Enterprise growth, value addition, market access and innovation across fashion, textiles and the creative economy. |
| Tourism and Hospitality | Enterprise development, supply chain opportunities and market linkages within tourism and hospitality. |

### Cross-cutting capabilities
- Leadership, governance, entrepreneurship, research and capacity development
  strengthen HerNext's work across sectors rather than operating as isolated
  areas of activity.
- Research and evidence support the full programme cycle: data informs
  programme design, implementation generates learning, measurement demonstrates
  results and credible evidence contributes to wider policy and institutional
  dialogue.

*These five were sectors before the correction. They are capabilities now, and
must not drift back into the sector list.*

### Current opportunities `#current-opportunities`
- **Heading:** What is open *today*
- **Lead:** HerNext periodically opens programmes, partnerships, trade
  opportunities, calls for participation and other initiatives aligned with our
  mission. Only opportunities that are currently active and accepting
  applications appear here.
- **Empty state:** **No opportunity is open at the moment.** Our initiatives are
  in development, and we do not list a programme here until it is genuinely
  accepting applications. Register your interest and we will contact you as
  opportunities open. HerNext never charges a fee to register or to take part.
- **Buttons:** Register your interest · Partner with HerNext

The partial carries an HTML comment showing the card markup for a real
opportunity. Remove a card the moment its deadline passes.

### Our operating philosophy
- **Statement:** Impact before visibility. Evidence before intervention.
  Sustainability before *scale*.
- **Body:** We listen before prescribing solutions. We use evidence to
  understand the barriers women face, develop interventions around real needs,
  measure what changes and scale approaches that demonstrate meaningful and
  sustainable impact.
- **Link:** See how we measure impact

---

## Impact

About explains who HerNext is. Our Work explains what HerNext does. Impact shows
what HerNext seeks to change, how change is measured, and what the evidence so
far demonstrates.

### Hero
- **Label:** Our impact
- **Heading:** Creating meaningful and lasting *change*
- **Lead:** At HerNext, impact is more than participation, activities or
  visibility. We measure success by the meaningful economic changes created in
  the lives, enterprises and livelihoods of the women we serve.

### What impact means to us
- **Heading:** From participation to *progress*
- **Lead:** Our work begins with evidence. We listen to women, identify the
  barriers affecting their economic progress, design practical interventions
  with the right partners and measure whether those interventions create
  measurable and sustainable change.
- **Callout:** **Our ambition is not simply to reach more women. It is to
  improve economic outcomes.** Numbers matter, but numbers alone do not tell the
  whole story. The number of women attending a programme tells us about reach.
  What happens afterwards tells us about impact. These are the six changes we
  look for.

| # | Change | Text |
|---|---|---|
| 01 | Increased income and profitability | Whether women are earning more and building more financially sustainable enterprises. |
| 02 | Improved productivity | Whether farmers, producers and entrepreneurs are able to produce more efficiently, improve output or reduce unnecessary production costs. |
| 03 | Better market access | Whether women gain reliable buyers, enter new markets, secure contracts or improve the value they receive for their products and services. |
| 04 | Access to finance | Whether women become better positioned to access appropriate financing, investment or other productive capital. |
| 05 | Stronger enterprises | Whether participating businesses improve their operations, resilience, financial management and capacity for sustainable growth. |
| 06 | Sustainable livelihoods | Whether interventions contribute to stronger and more resilient sources of income for women and their households. |

### Our impact model
- **Heading:** Evidence before *intervention*
- **Pathway:** Listen → Baseline → Identify → Design → Partner → Implement →
  Measure → Learn and scale

| # | Step | Text |
|---|---|---|
| 01 | Listen | We begin with the women, enterprises and communities we seek to serve. |
| 02 | Establish the baseline | We collect information that helps us understand the current situation before an intervention begins. |
| 03 | Identify the barrier | We use evidence to determine what is preventing economic progress. |
| 04 | Design the intervention | We develop practical responses around demonstrated needs rather than assumptions. |
| 05 | Mobilise partners | We bring together organisations with the technical expertise, markets, financing, technology or other resources required. |
| 06 | Implement | We deliver targeted interventions with clear objectives and intended outcomes. |
| 07 | Measure change | We compare results against the baseline and assess what actually changed. |
| 08 | Learn and scale | We strengthen what works, learn from what does not and scale interventions that demonstrate sustainable impact. |

### Impact in action
- **Heading:** Starting with *evidence*
- **HerNext Kiambu Chapter.** HerNext's community-level approach begins by
  listening. Following the establishment of the chapter, women from different
  economic activities were engaged to understand their enterprises, livelihoods
  and most significant barriers to growth.
- Individual follow-up and baseline data collection were subsequently undertaken
  with women engaged in livestock enterprises, including dairy, poultry and pig
  farming. The assessment identified animal feed affordability and access as a
  major recurring challenge affecting participating livestock farmers.
- Rather than immediately prescribing a solution, HerNext is using these
  findings to inform the design of a targeted intervention and identify
  appropriate technical, commercial and development partners. The intervention
  will be designed to assess changes in production costs, productivity,
  profitability, market readiness and household livelihoods.
- **Callout:** This is the HerNext approach in practice: listen first,
  understand the evidence, intervene strategically and measure what changes.

**Kiambu is an example of the process, not proof of completed economic impact.
No outcome may be attributed to it until follow-up measurement is done.**

### From data to action
- **Heading:** What happens *next*
- Once a challenge has been identified, HerNext seeks partners capable of
  addressing different parts of the problem.
- In livestock, for example, collaboration may include animal nutrition and feed
  companies, veterinary and technical experts, financial institutions,
  agricultural research organisations, government and county institutions,
  development partners, processors and buyers.
- HerNext's role is to organise, connect, coordinate and measure, building an
  ecosystem around the economic challenge rather than attempting to solve every
  problem alone.

### How we measure impact
- **Heading:** Accountability through *evidence*
- **Lead:** Each HerNext programme establishes relevant baseline indicators
  before implementation and measures progress over time.

| Domain | Indicators |
|---|---|
| Enterprise | Revenue growth, profitability, business survival, jobs created or sustained. |
| Agriculture | Production levels, input and feed costs, yield and productivity, mortality rates, farmer income. |
| Markets | Number of reliable buyers, sales volumes, value of contracts, new markets accessed. |
| Finance | Women accessing finance, value of finance accessed, investment readiness. |
| Trade | Businesses entering new markets, buyer connections, export opportunities converted into actual business. |

- **Callout:** We are committed to reporting both what works and what we learn,
  using evidence to strengthen future interventions. No figure appears on this
  page until follow-up measurement supports it.

### Stories of change
- **Heading:** Behind every number is a *woman*
- As HerNext interventions are implemented and evaluated, this space will
  document verified stories of women whose enterprises, incomes and livelihoods
  have changed through access to the right opportunities and partnerships.
- Stories are published only with the permission of the women they describe, and
  supported wherever possible by verified baseline and follow-up information.
  Until then, this space stays deliberately empty.

### Sustainable Development Goals **[profile]**
- **Heading:** Contributing to global and African development *priorities*
- **Lead:** Our interventions and partnerships align closely with five of the
  Sustainable Development Goals, while remaining committed to solutions that
  reflect Africa's own realities, priorities and ambitions.

| Goal | Text |
|---|---|
| SDG 5, Gender Equality | Supporting women's leadership, entrepreneurship and economic participation. |
| SDG 8, Decent Work and Economic Growth | Promoting enterprise development, innovation and inclusive economic growth. |
| SDG 9, Industry, Innovation and Infrastructure | Encouraging industrial development, value addition and technological innovation. |
| SDG 13, Climate Action | Promoting environmentally responsible business practices and sustainable development. |
| SDG 17, Partnerships for the Goals | Strengthening collaboration between governments, development partners, businesses, academia and civil society. |

### Our commitment
- **Statement:** Impact before visibility. Evidence before intervention.
  Sustainability before *scale*.
- **Body:** We believe visibility matters when it brings attention, resources
  and partnerships to meaningful work. But our ultimate measure of success is
  not how visible HerNext becomes. It is whether the women we work alongside are
  building stronger enterprises, accessing better markets, increasing their
  incomes and creating more sustainable livelihoods. That is the impact we seek
  to build.

**Removed by the 2026 correction and not to be restored:** the "Understanding
the opportunity gap" section, which About now covers; the five principles
(Creating Opportunities, Strengthening Institutions, Building Partnerships,
Promoting Innovation, Advancing Sustainable Development), which the new About
philosophy and the Our Work method supersede; and "Outcomes, not activities",
whose eight-item list is replaced by the six changes and the five measurement
domains above.

---

## Partners

### Hero
- **Label:** Partners and collaboration
- **Heading:** Strong partnerships create lasting *impact*
- **Lead:** Collaboration is central to how HerNext works. We believe complex
  economic barriers cannot be solved by one institution alone. Meaningful and
  sustainable progress happens when organisations bring together complementary
  expertise, resources, markets, knowledge and capabilities around clearly
  identified needs.

### Our partnership philosophy
- **Heading:** Different strengths. Shared *purpose*
- No single organisation holds every solution. A financial institution may
  provide capital. A business may provide access to markets. A technology
  company may provide tools. A development partner may provide technical
  expertise and resources. Government may strengthen the enabling environment.
  Communities provide lived experience and local knowledge.
- HerNext brings these capabilities together around the barriers identified
  through our work.
- We seek partnerships founded on trust, transparency, shared values and mutual
  accountability, with a preference for relationships capable of creating
  sustainable value beyond a single activity or event. By connecting women,
  institutions, expertise and opportunity, we seek to strengthen the economic
  ecosystems in which women build enterprises and livelihoods.
- **Callout:** HerNext builds partnerships that turn shared purpose into
  practical action and measurable economic outcomes for women.

### Who we partner with
- **Heading:** Different capabilities, one shared *goal*

| # | Partner | Text |
|---|---|---|
| 01 | Private sector | Businesses can provide markets, supply chain opportunities, technology, technical expertise, products and commercial partnerships that strengthen women's enterprises. |
| 02 | Financial institutions and investors | Banks, SACCOs, investment organisations and other capital providers can expand access to appropriate finance, strengthen financial capability and support investment-ready enterprises. |
| 03 | Development partners and foundations | Development institutions, foundations and philanthropic organisations can contribute funding, technical expertise, research capability and programme support that enable evidence-driven interventions to grow. |
| 04 | Government and public institutions | National and county governments, agencies and public institutions can strengthen enabling environments, provide technical support and connect interventions with wider economic development priorities. |
| 05 | Academic, research and technical institutions | Universities, research organisations and technical institutions provide evidence, specialised expertise, innovation and knowledge that strengthen programme design, implementation and measurement. |
| 06 | Civil society and community organisations | Community-based and civil society organisations help ensure interventions remain locally grounded, inclusive and responsive to the realities of the people they are designed to serve. |
| 07 | Market and trade partners | Buyers, distributors, retailers, exporters, trade organisations and industry associations can create pathways from production to sustainable markets. |
| 08 | Entrepreneurs and industry experts | Entrepreneurs, professionals, mentors and technical specialists contribute experience, knowledge, innovation and practical expertise that can strengthen women-owned enterprises and livelihoods. |

*Seven categories before the correction, eight now. Market and trade partners is
new, and development partners was split from foundations.*

### How we partner **[new section, client copy]**
- **Heading:** From collaboration to measurable *action*
- **Lead:** Collaboration takes seven practical forms, and most partnerships
  combine more than one.

| # | Form | Text |
|---|---|---|
| 01 | Programme partnerships | Co-designing, funding or implementing interventions responding to demonstrated economic needs. |
| 02 | Market partnerships | Connecting women-owned enterprises, producers and farmers to buyers, distributors, supply chains and new markets. |
| 03 | Finance and investment partnerships | Creating pathways to appropriate finance, investment and financial capability. |
| 04 | Technical partnerships | Providing specialist expertise, technology, training, research or technical assistance. |
| 05 | Resource and funding partnerships | Supporting evidence-driven programmes through grants, sponsorship, catalytic funding or other appropriate resources. |
| 06 | Research and impact partnerships | Supporting baseline studies, monitoring, evaluation, learning and evidence generation. |
| 07 | Trade and international partnerships | Creating opportunities for trade missions, buyer engagement, investment connections and regional or international market access. |

### How a HerNext partnership begins
- **Heading:** Start with the *need*
- **Pathway:** Need identified → Evidence collected → Intervention designed →
  Partners mobilised → Programme implemented → Impact measured
- HerNext partnerships begin with a clearly identified challenge or opportunity.
  Where possible, we use evidence from women, enterprises and communities to
  understand the problem before determining the partnership required.
- We then identify organisations whose capabilities can contribute meaningfully
  to the solution, establish clear roles and intended outcomes, implement
  collaboratively and measure what changes.
- **Callout:** We seek partners around real problems, not partnerships simply
  for visibility.

### Partnership principles
- **Heading:** Every partnership is guided by five *principles*

| # | Principle | Text |
|---|---|---|
| 01 | Shared purpose | We build partnerships around clearly defined objectives and outcomes that create meaningful value. |
| 02 | Mutual respect | We recognise and value the expertise, perspective and contribution each partner brings. |
| 03 | Transparency and accountability | We promote openness, ethical practice, responsible stewardship and accountability for commitments and resources. |
| 04 | Sustainability | We prioritise solutions capable of creating value beyond a single activity or funding cycle. |
| 05 | Measurable impact | We seek to understand what changed as a result of our collaboration and use evidence to strengthen future work. |

*Principle 05 was "Innovation" before the correction. Innovation remains
important within HerNext's work but is not a principle of every partnership.*

### Partner with HerNext
- **Heading:** Let us build something *meaningful*
- **Body:** If your organisation shares our commitment to expanding women's
  economic opportunity and has resources, expertise, markets, technology,
  finance or networks that can help address real economic barriers, we would
  welcome a conversation. Whether through programme collaboration, market
  access, technical expertise, funding, research or investment, we are
  interested in partnerships capable of moving from intention to measurable
  impact.
- **Buttons:** Partner with HerNext · Explore our work

**No partner logo may appear on this page unless HerNext has an actual
relationship with the organisation and permission to use its branding. Keep
partnership claims factual: no language suggesting funding, endorsement or
formal collaboration where discussions are only exploratory.**

---

## Contact

The contact page carries the only form on the site. It absorbed the four-step
registration flow that used to live at `apply.html`.

- **Label:** Contact
- **Heading:** Start a *conversation*
- **Lead:** Whether you are registering your interest, exploring a partnership
  or requesting information about our work, we would be glad to hear from you.
- **Contact details:** info@hernextnetwork.com · hernextnetwork@gmail.com ·
  0780 528 551 · 0734 806 637
- **Follow:** Instagram @hernextnetworkltd · LinkedIn *(placeholder)*
- **Registering your interest:** Our initiatives are in development and none is
  currently accepting applications. Choose *Register your interest* below and
  tell us what you are building, the sector you work in and where you are based,
  and we will contact you as opportunities open. We never charge a fee to
  register or to take part. **[new]**
- **Partnership enquiries:** Institutions exploring collaboration are welcome to
  write directly. Please include your organisation, the area of interest and the
  outcome you have in mind, and the relevant member of our team will respond.
  **[new]**

### Enquiry form **[new]**
- **Heading:** How can we *help*?
- Full name `*` · Email address `*` · Organisation · Subject `*` · Message `*`
- **Subject options:** Register your interest · Partnership enquiry · Market,
  trade or buyer connection · Media and speaking · Research and policy ·
  General enquiry
- **Button:** Send enquiry
- **Success:** Thank you. Your enquiry has been sent, and we will respond shortly.

### Validation messages **[new]**
| Case | Message |
|---|---|
| Empty required text field | This field is required. |
| Empty required select | Please make a selection. |
| Invalid email | Please enter a valid email address. |
| Form not yet connected | This form is not connected yet. Add your Formspree form ID to the data-endpoint attribute, as described in the README. |
| Sending | Sending, one moment. |
| Submission failed | Your message could not be sent. Please try again, or email us directly at info@hernextnetwork.com. |

---

## Page not found

- **Label:** Error 404
- **Heading:** This page has *moved on*
- **Body:** The page you were looking for is not here. It may have been renamed,
  or the link that brought you may be out of date.
- **Buttons:** Return home · Explore our work
- **Section:** Where you may have been *heading*, with cards linking to About,
  Our Work, Impact and Contact.

---

## Moved addresses

Two stub pages stand where content used to live, so a link already shared still
lands. Both are generated by `tools/make.py`.

| Address | Goes to | Heading |
|---|---|---|
| `opportunities.html` | `our-work.html` | Opportunities is now Our Work |
| `apply.html` | `contact.html` | Registering your interest moved |

---

## Legal pages

Three pages written for this site, all **[new]**, all drafts pending review by
a qualified adviser. Each carries a visible note saying so, and each marks the
placeholders that must be filled in: `[registered entity name]`,
`[registered address]` and `[governing jurisdiction]`.

### Privacy policy *(privacy.html)*
Eleven sections: who we are, what we collect, why and on what basis, who else
handles it, where it goes, how long we keep it, your rights, security, children,
changes, governing law.

Written to describe what the site genuinely does rather than a generic template.
It names the three third parties that actually see visitor data, Formspree,
GitHub Pages and Google Fonts, states that the site sets no cookies of its own
and runs no analytics, and gives retention periods of twenty four months for
enquiries and thirty six for registrations. **If Google Fonts is ever
self-hosted, or analytics added, section 4 must change with it.**

Section 2 describes one form, not two. When the four-step application flow was
folded into the contact form, the field list narrowed to name, email,
organisation, subject and message. **If a longer intake form ever returns, that
list must grow with it.**

### Terms of use *(terms.html)*
Thirteen sections. Two carry most of the weight:

- **Section 2, the status of our programmes.** States in a boxed callout that
  the initiatives described are statements of intent, that registering is not an
  application to an open programme and creates no obligation, and that nothing
  on the site is an offer of employment, funding, investment, a grant or a
  scholarship.
- **Section 3, no fees and how to spot impersonation.** States that HerNext
  never charges to register or take part, names the only official channels
  (this website, info@hernextnetwork.com, hernextnetwork@gmail.com and Instagram
  @hernextnetworkltd), and gives an address to report anyone soliciting payment
  in its name.

### Accessibility statement *(accessibility.html)*
Commits to WCAG 2.1 AA, lists ten specific measures taken, and names four known
limitations honestly: the Google Fonts dependency, screen reader pairings not
yet exhaustively tested, photography still being replaced, and no independent
audit yet. Gives a contact route and a ten working day response aim.

---

## Image alt text

All fourteen slots carry HerNext Network photography from a single community
gathering. Alt text describes what is in the frame, not what the section is
about. The same fourteen strings appear in the partials; these two lists move
together.

| Slot | Alt text |
|---|---|
| Home hero | Women standing among rows of plastic chairs in a community hall, under a high corrugated roof |
| About, our story | Two people in HerNext Network shirts standing to address a seated group in a community hall |
| Partners banner | A wide view of a community hall, rows of women seated facing two people standing at a plywood partition |
| Leadership Academy card | A woman speaking from a sheet of notes while a colleague in a HerNext Network shirt listens |
| Opportunity Hub card | A man in a HerNext Network shirt standing with folded arms, talking with women seated around him |
| Global Trade card | A woman standing beside a cloth covered table, addressing people seated in a sunlit hall |
| Impact, Kiambu Chapter | Rows of women seated in a community hall, seen from the back, facing a speaker at the far end |
| Contact | Two women in HerNext Network shirts in conversation, one holding a sheet of notes |
| Closing call to action | A woman standing with a sheet of notes, speaking to women seated around her |
| Home, why we exist | Three women standing together on a compound path, two of them in HerNext Network shirts |
| Home, how we work | A woman in a HerNext Network shirt speaking, a sheet of notes in her hand, in front of a plywood wall |
| Partners, how a partnership begins | Two men shaking hands at a gateway while two women stand alongside them |
| Impact, from data to action | A woman in a HerNext Network shirt holding a phone, smiling, beside a window |
| About, the Africa we envision | Three people in HerNext Network shirts standing hand in hand with heads bowed, in a circle with others |

The social preview carries the home hero string, in `og:image:alt` and
`twitter:image:alt`, because `og-image.jpg` is recut from the hero crop. If the
hero photograph changes, three strings change together.

The previous set described stock and conference imagery that the site no longer
uses. The rule that produced these is unchanged: describe the frame, and never
name an initiative or imply that a particular event took place. The partners
banner once named "the HerNext Partnership Forum", an initiative the
corrections removed, and naming an event that may never have run under that
name would imply it did.

The header logo carries an empty `alt` because the brand name sits beside it as
text. The footer logo carries `alt="HerNext Network"`.

The home hero also carries a looping clip over its photograph. The clip is
marked `aria-hidden` and carries no alternative text of its own, deliberately:
it shows nothing the photograph underneath does not already show, and that
photograph keeps the alt text in the table above. Describing both would make a
screen reader read the same scene twice. The accessibility statement records
that the clip is silent, stoppable, and never loaded when reduced motion is
asked for.

---

## Open questions for HerNext

1. Registered entity name, registered address and governing jurisdiction, for
   the privacy policy and the terms.
2. The LinkedIn URL. Instagram is confirmed as @hernextnetworkltd.
3. Real impact figures, when available, for the statistics band on the home
   page and for the Impact page. Nothing goes up before follow-up measurement.
4. Whether the Kiambu livestock intervention has a named partner set yet, and
   whether it may be described.
5. Whether to publish an indicative timeline for the first initiative, once
   funding is settled. Until then the site deliberately commits to none.
6. Whether the structured four-step intake should return once initiatives open.
   It was removed in 2026 and is recoverable from git history at commit
   `d7fd658`.
